  ...
  static char mks_version[] = "@(#) "UFIS_VERSION" $Id: Ufis/_Standard/_Standard_Server/Base/Server/Kernel/action.c 1.20 2013/08/23 10:29:17CEST fya Exp  $";
			   ...
			   char *pcpSelection, char *pcpFields,char *pcpData, int ipTableNo)
{
  ...
  ilDatLstLen= (strlen(pcpData) * 2) + 128;
		...
		while ((pclRulEnd != NULL) && (ilBreakOut == FALSE))
		{
			...
			if (ilLen > 0)
			{
				...
				if (ilFldNbr > 0)
				{
						...
						if (strstr(",TC,TCM,TCH,TCD,DC,DCM,DCH,DCD,DCA,DCAU", pclChkKey) != NULL)
						{
							if (ilDatLen > 0)
							{
							 ilKeys++;
                              ...
                              char *pcpData, char *pcpOldData,int ipTableNo)
{
  ...
  /* FYA 20130822 v1.24
  char pclNewDat[2048] = "\0";
  char pclOldDat[2048] = "\0";
  */
				...
				for(ilCurrentField = 0; ilCurrentField< rgTM.prSection[ipTableNo].iNoOfFields; ilCurrentField++) 
				{
					...
					if(!strcmp(rgTM.prSection[ipTableNo].pcFields[ilCurrentField],pclFldNam))
					{
						...
						if (strcmp(pclOldDat,pclNewDat) != 0 || rgTM.prSection[ipTableNo].pcFieldResponseType[ilCurrentField] == '!')
						{
							...
							//dbg(DEBUG,"+++++++before<%d>",ilCurrentField);
							...
							//dbg(DEBUG,"+++++++after<%d>",ilCurrentField);
