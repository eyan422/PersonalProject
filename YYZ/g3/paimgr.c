...
/* Author         : won                                                         */
/* Date           : 20121218                                                    */
/* Description    : UFIS-2770                                                   */
/*                   - Honeywell Connection Manager for outgoing data from UFIS */
/*                                                                              */
/* Update history :                                                             */
...
/*   Database Field Size   */
#define MAX_paitab_URNO 14
#define MAX_paitab_DATA 8192
#define MAX_paitab_STAT 8
#define MAX_paitab_DATE 14
#define MAX_paitab_MGID 5
...
typedef struct
{
    char URNO[MAX_paitab_URNO+1];
    char DATA[MAX_paitab_DATA+1];
    char STAT[MAX_paitab_STAT+1];
    char CDAT[MAX_paitab_DATE+1];
    char SDAT[MAX_paitab_DATE+1];
    char ADAT[MAX_paitab_DATE+1];
    char DDAT[MAX_paitab_DATE+1];
    char MGID[MAX_paitab_MGID+1];
}paitab;
...
char pcgCurrentpaitabUrno[32]="";
char pcgNewpaitabUrno[32]="";
int igConn1RecStatus=IDLE; /* hwecon1 record status */
int igConn2RecStatus=IDLE; /* hwecon2 record status */

paitab rgpaitabRec; /* placeholder for a record in paitab */
char *pcgpaitabFields="URNO,DATA,STAT,CDAT,SDAT,ADAT,DDAT,MGID";
int igpaitabFieldCount=8;
...
int getNextpaitabRecord(paitab *rppaitabRec);
...
static int InitPgm();
int insertpaitab(char *pcpData, char *pcpMGID);
void printConfig();
void printpaitabRecord(paitab *rppaitab);
int processCDB();
int processCONX();
int processDROP();
int processNACK();
...
int updatepaitab(char *pcpWhere, int ipArgc,...);
...
MAIN
{
    ...
    /*
    getNextpaitabRecord(&rgpaitabRec);
    printpaitabRecord(&rgpaitabRec);
    */

    strcpy(pclSql, "DELETE FROM paitab ");
    ...
    ilRC = sendCedaEventWithLog(rgConfig.iConn1, 0, pcgDestName, pcgRecvName,
                                pcgTwStart, pcgTwEnd, "CONX", "paitab", "",
                                "", "",
                                "", 3, NETOUT_NO_ACK);
...
void printConfig()
{
    ...
    dbg(TRACE, "printConfig:: paitab Retention in Days [%d]", rgConfig.ipaitabRetention);
...
int getConfig(char *pcpFileName)
{
    ...
    ilRC=ReadConfigEntry("MAIN","paitab_RETENTION_DAYS", pclTmp, "5");
    if ( ilRC==RC_FAIL )
    {
    ...
    {
        rgConfig.ipaitabRetention=atoi(pclTmp);
    }
    ...
    if (( ilRC == RC_SUCCESS) && (strcmp(pclTmp, "YES") == 0) )
    {
        ...
        igReSendEnable = FALSE;

    if (igReSendEnable == TRUE)
    {
        ilRC = ReadConfigEntry("MAIN","RESEND_MAX", pclTmp, "3");
        if ( ilRC == RC_FAIL )
            igReSendMax = 3;
        else
...
processCDB: Perform actions for CDB commands:  Delete all records whose cdat
            ...
            paitab_RETENTION_DAYS.  If this variable is not found it is set to
...
int processCDB()
{
    ...
    ilRC=getRetentionDate(rgConfig.ipaitabRetention, pclDate);
    sprintf(pclSql,"DELETE FROM PAITAB WHERE cdat<'%s'", pclDate);
...
int processXMLO(char *pcpData, char *pcpMGID)
{
    ...
    dbg(TRACE, "processXMLO:: Final: paitab Urno[%s], Connection Mask[%d], igConn1RecStatus[%d], igConn2RecStatus[%d]",
        pcgCurrentpaitabUrno, cgConnectionMask, igConn1RecStatus, igConn2RecStatus);
...
prcessRST:  Process the RST command.
             1. Set outstanding records in paitab to status DROP.
             2. Send to hwecon1/hwecon2
*******************************************************************************/
int processRST()
{
    ...
    if ( !strcmp(pcgCurrentpaitabUrno, "") )
        strcpy(pclWhere, "WHERE stat=' ' ");
    else
        sprintf(pclWhere, "WHERE stat=' ' or URNO='%s' ", pcgCurrentpaitabUrno);
    ...
    if ( ilRC!=RC_FAIL )
    {
    	...
    	ilTmp1=sendCedaEventWithLog(rgConfig.iConn1, 0, pcgDestName, pcgRecvName,
                                    pcgTwStart, pcgTwEnd, "DROP", "paitab",
                                    "", "", "",
                                    "", 3, NETOUT_NO_ACK);
...
/*******************************************************************************
insertpaitab: Insert a record in paitab.
*******************************************************************************/
int insertpaitab(char *pcpData, char *pcpMGID)
{
    int ilRC=RC_SUCCESS;
    ...
    getSystemDate(pclDate);
    ilUrno=NewUrnos("paitab",1);

    sprintf(pcgNewpaitabUrno, "%2.2s%2.2s%d",
    ...
    sprintf(pclSql, "INSERT INTO PAITAB "
                    ...
                    "VALUES "
                    "('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')",
            pcgNewpaitabUrno, pclTmp, " ", pclDate, " ", " ", " ", pcpMGID);

    ilRC=executeSql(pclSql, pclSqlErr);
    dbg(TRACE,"insertpaitab:: SQL Result[%d][%s]", ilRC, pclSqlErr);
    if ( ilRC==RC_FAIL )
    {
        dbg(TRACE,"insertpaitab:: DB error!");
    }
    else
    {
        dbg(TRACE, "insertpaitab:: Record Added Successfully");
    }

    dbg(DEBUG, "insertpaitab:: End Result[%d]", ilRC);
    return ilRC;
}

/*******************************************************************************
updatepaitab:  Update a record in paitab using pcpUrno as key.  Arguments must
...
int updatepaitab(char *pcpWhere, int ipArgc, ...)
{
    ...
    dbg(DEBUG, "updatepaitab:: Start");
    dbg(DEBUG, "updatepaitab:: Condition[%s], Fields to be updated [%d]", pcpWhere, ipArgc);

    if ( ipArgc<1 )
    {
        dbg(TRACE, "updatepaitab:: No field to update!");
    }
    else if ( (ipArgc%2)==1 )
    {
        dbg(TRACE, "updatepaitab:: Field/Value pair not match. ");
        ilRC=RC_FAIL;
    }
    else
    {
        memset(&pclSql, 0, sizeof(pclSql));
        strcpy(pclSql, "UPDATE PAITAB SET ");
        va_start(valArgv, ipArgc);
        /*
        for ( ilCounter = 0; ilCounter< ipArgc; ilCounter++ )
           dbg(DEBUG, "updatepaitab:: Count[%d][%s]", ilCounter, va_arg(valArgv, char *));
        */
        ...
        ilRC=executeSql(pclSql, pclSqlErr);
        dbg(TRACE,"updatepaitab:: executing [%s][%d][%s]", pclSql, ilRC, pclSqlErr);

        if ( ilRC==RC_FAIL )
        {
            dbg(TRACE,"updatepaitab:: DB error!");
        }
        else if ( ilRC==1 )
        {
            dbg(TRACE, "updatepaitab:: No Record to update");
        }
        else
        {
            dbg(TRACE, "updatepaitab:: Record(s) Updated Successfully");
        }
    }
    dbg(DEBUG, "updatepaitab:: End Result[%d]", ilRC);
    return ilRC;
}
...
processDROP: Process DROP command.
             ...
             2.  If there is a change from non zero to zero in connection mask,
                 set all records in paitab with stat=' ' or 'SENT' to drop.
*******************************************************************************/
int processDROP()
{
    int ilRC=RC_SUCCESS;
    ...
    //Frank
	//if there is only one connection and received the corresponding NOACK from it when the DROP command comes, then hwemgr should gets the next record in paitab which is similar to processNACK
...
/*
		...
		if ( ilRC==RC_SUCCESS )
		{
		    ilRC=sendDataHweConn(rgpaitabRec.URNO, rgpaitabRec.DATA);
		}
		else if ( ilRC==1 )
		{
		    ilRC=RC_SUCCESS;
...
int processRACK(char *pcpData, char *pcpMGID)
{
    ...
    {
        getSystemDate(pclDate);
        ilRC = updatepaitab(pclWhere, 4, "STAT", "ACK", "ADAT", pclDate);
    }
    else
    {
        sprintf(pclSql,"DELETE FROM PAITAB %s", pclWhere);
        ilRC = executeSql(pclSql, pclSqlErr);
    ...
    if ( ilRC==RC_SUCCESS )
    {
        ilRC = sendDataHweConn(rgpaitabRec.URNO, rgpaitabRec.DATA, rgpaitabRec.MGID, "0");
    }
    else if ( ilRC==1 )
    {
        ilRC=RC_SUCCESS;
...
/*******************************************************************************
processASNT: Process ASNT command.
             1.   Msg was Sent by con
*******************************************************************************/
int processASNT(char *pcpData, char *pcpMGID)
{
    int ilRC=RC_SUCCESS;
    char pclDate[16];
    char pclWhere[1024];
...
/*******************************************************************************
getNextpaitabRecord:  Get the next record in paitab.  STAT=' ' order by URNO
*******************************************************************************/
int getNextpaitabRecord(paitab *rppaitabRec)
{
    int ilRC=RC_SUCCESS;
    ...
    dbg(TRACE, "getNextpaitabRecord:: Start");

    memset(rppaitabRec, 0, sizeof(paitab));
    ...
    dbg(TRACE, "getNextpaitabRecord:: Executing [%s]", pclSql);
    ilRC = sql_if (START, &slCursor, pclSql, pclDataBuf);

    if( ilRC == 0 )
    {
        BuildItemBuffer (pclDataBuf, pcgpaitabFields, igpaitabFieldCount, ",");
        get_real_item(rppaitabRec->URNO,pclDataBuf,1);
        get_real_item(rppaitabRec->DATA,pclDataBuf,2);
        ConvertDbStringToClient(rppaitabRec->DATA);
        get_real_item(rppaitabRec->STAT,pclDataBuf,3);
        get_real_item(rppaitabRec->CDAT,pclDataBuf,4);
        get_real_item(rppaitabRec->SDAT,pclDataBuf,5);
        get_real_item(rppaitabRec->ADAT,pclDataBuf,6);
        get_real_item(rppaitabRec->DDAT,pclDataBuf,7);
        get_real_item(rppaitabRec->MGID,pclDataBuf,8);
    }
    else if ( ilRC==1 )
    {
        dbg(TRACE,"getNextpaitabRecord:: No record found");
    }
    else
        dbg(TRACE,"getNextpaitabRecord:: DB ERROR");

    close_my_cursor(&slCursor);

    dbg(TRACE, "getNextpaitabRecord:: End Result[%d]", ilRC);
    return ilRC;
}

/*******************************************************************************
printpaitabRecord:  For debugging purposes.  Print paitab record.
*******************************************************************************/
void printpaitabRecord(paitab *rppaitab)
{
    dbg(DEBUG, "printpaitabRecord:: Start");

    dbg(DEBUG, "printpaitabRecord:: paitab");
    dbg(DEBUG, "printpaitabRecord::      URNO[%s]", rppaitab->URNO);
    dbg(DEBUG, "printpaitabRecord::      DATA[%s]", rppaitab->DATA);
    dbg(DEBUG, "printpaitabRecord::      STAT[%s]", rppaitab->STAT);
    dbg(DEBUG, "printpaitabRecord::      CDAT[%s]", rppaitab->CDAT);
    dbg(DEBUG, "printpaitabRecord::      SDAT[%s]", rppaitab->SDAT);
    dbg(DEBUG, "printpaitabRecord::      ADAT[%s]", rppaitab->ADAT);
    dbg(DEBUG, "printpaitabRecord::      DDAT[%s]", rppaitab->DDAT);
    dbg(DEBUG, "printpaitabRecord::      MGID[%s]", rppaitab->MGID);

    dbg(DEBUG, "printpaitabRecord:: End");
    return;
}
...
/*******************************************************************************
sendDataHweConn:  Update record in paitab to STAT='SENT'.
...
int sendDataHweConn(char *pcpUrno, char *pcpData, char *pcpMGID, char *pcpSndCnt)
{
             ...
             || (igConn2RecStatus == SENT && igSendWaitAck == FALSE) )) )
    {
        ...
        if (igReSendEnable == TRUE)
        {
            ...
            ilRC = updatepaitab(pclWhere, 8, "stat", "SENT", "sdat", pclDate, "SCNT", pclNextScnt, "ESDT", pclExpDate);
        }
        else
            ilRC = updatepaitab(pclWhere, 4, "stat", "SENT", "sdat", pclDate);
        ...
        if ( ilRC==RC_SUCCESS )
        {
            ...
            {
                ilTmp1 = sendCedaEventWithLog(rgConfig.iConn1, 0, pcgDestName, pcgRecvName,
                                         pcgTwStart, pcgTwEnd, "XMLO", "paitab", pcpMGID,
                                         "DATA", pcpData,
                                         "", 3, NETOUT_NO_ACK);
            ...
            strcpy(pcgCurrentpaitabUrno, pcpUrno);
        }
/*
        if ( (cgConnectionMask&1)==1 )
        {
        	...
        	ilRC = sendCedaEventWithLog(rgConfig.iConn1, 0, pcgDestName, pcgRecvName,
                                        pcgTwStart, pcgTwEnd, "XMLO", "paitab", "",
                                        "DATA", pcpData,
                                        "", 3, NETOUT_NO_ACK);
            igConn1RecStatus=SENT;
            ilSend=1;
...
int processSTAT()
{
    ...
    dbg(TRACE, "processSTAT:: Connection 1 Record Status[%d], Connection 2 Record Status[%d]",
        igConn1RecStatus, igConn2RecStatus);
    dbg(TRACE, "processSTAT:: Current paitab URNO in Process[%s]", pcgCurrentpaitabUrno);
...
int processDLY()
{
    ...
    char pclCurTim[16] = "\0";
    char pclUrno[32] = "\0";
    char pclData[MAX_paitab_DATA] = "\0";
    char pclMgid[32] = "\0";
    char pclScnt[16] = "\0";
    ...
    sprintf(pclSqlBuf,"SELECT URNO,SCNT,ESDT FROM PAITAB WHERE "
    ...
    {
        /*fya v1.5*/
        dbg(TRACE, "%s ++++MAX RE SEND EXCEED FOR NO ACK, INI AGAIN->Delete paitab and reinit hweexco and hwepde+++++", pclFunc);
    	strcpy(pclSqlBuf, "DELETE FROM PAITAB ");
    ...
    /*sprintf(pclSqlBuf,"SELECT URNO,DATA,MGID,SCNT,ESDT FROM paitab WHERE "
    ...
    pclDatBuf[0] = 0x00;
    slFkt = START;
    while((ilGetRc = sql_if(slFkt,&slCursor,pclSqlBuf,pclDatBuf)) == DB_SUCCESS)
    {
        if (ilGetRc == DB_SUCCESS)
        {
            BuildItemBuffer (pclDatBuf, "", 5, ",");
            get_real_item(pclUrno,pclDatBuf,1);
            get_real_item(pclData,pclDatBuf,2);
    ...
    /* Send all the Time New Msg */
    if (igSendWaitAck == FALSE)
    {
        sprintf(pclSqlBuf,"SELECT URNO,DATA,MGID,SCNT,ESDT FROM PAITAB WHERE STAT = ' ' ") ;
        dbg(TRACE,"%s \n%s",pclFunc, pclSqlBuf);
        slCursor = 0;
        pclDatBuf[0] = 0x00;
        slFkt = START;
