  ...
  static char mks_version[] = "@(#) "UFIS_VERSION" $Id: Ufis/_Standard/_Standard_Server/Base/Server/Interface/gathdl.c 1.5 2014/12/10 14:11 PM Exp  $";
...
static int StoreSentData(char *pcpDataSent, char *pcpUaft, SENT_MSG rpMsg, char *pcpRecordURNO, char *pcpType, char *pcpStat);
...
static void GetFlightWhereClause(char *pcpWhere);
static void GetFlightFullQuery(char *pcpSqlBuf,char *pcpFieldList, char *pcpWhere);
...
static void GetFlightWhereClause(char *pcpWhere)
{
	char *pclFunc = "GetFlightWhereClause";
	char pclWhere[2048] = "\0";

	sprintf(pclWhere,"WHERE (ADID = 'A' AND %s BETWEEN '%s' AND '%s' AND TRIM(GTA1) IS NOT NULL) OR (ADID = 'D' AND %s BETWEEN '%s' AND '%s' AND TRIM(GTD1) IS NOT NULL)",pcgTimeWindowRefField_AFTTAB_Arr, pcgTimeWindowLowerLimit,pcgTimeWindowUpperLimit,pcgTimeWindowRefField_AFTTAB_Dep ,pcgTimeWindowLowerLimit,pcgTimeWindowUpperLimit);
...
static void GetFlightFullQuery(char *pcpSqlBuf,char *pcpFieldList, char *pcpWhere)
{
	char *pclFunc = "GetFlightFullQuery";
	char pclSqlBuf[2048] = "\0";

	sprintf(pclSqlBuf, "SELECT %s FROM AFTTAB %s", pcpFieldList, pcpWhere);
...
static int HandleInternalData()
{
	...
	*/
	...
	char pclArrRefValue[16] = "\0";
	char pclDepRefValue[16] = "\0";
	...
	else if (strcmp(cmdblk->command,"UFR") == 0 || strcmp(cmdblk->command,"IFR") == 0 || strcmp(cmdblk->command,"DFR") == 0)
	{
        ...
        if( getFieldValue(pclFields, pclNewData, pcgTimeWindowRefField_AFTTAB_Arr, pclArrRefValue) == RC_FAIL )
            return RC_FAIL;

        if( getFieldValue(pclFields, pclNewData, pcgTimeWindowRefField_AFTTAB_Dep, pclDepRefValue) == RC_FAIL )
            return RC_FAIL;
		...
		{
			case 'A':
                if( isInTimeWindow(pclArrRefValue, pcgTimeWindowLowerLimit, pcgTimeWindowUpperLimit) == RC_FAIL )
                {
                    return RC_FAIL;
                }
				...
				break;
			case 'D':
                if( isInTimeWindow(pclDepRefValue, pcgTimeWindowLowerLimit, pcgTimeWindowUpperLimit) == RC_FAIL )
                {
                    return RC_FAIL;
                }
		...
		StoreSentData(pclDataSent,pclUrnoNewData,rlSentMsg,pclRecordURNO,"UPDATE","SENT");
			...
			else
			{
				if(FD_ISSET( ipSock, &readfds))
				{
                    ...
                    if(strcmp(pcgTest,"YES") == 0)
                    {
                        ...
                        strcpy(pclRecvBuffer,"ACK_ufis................2014120909084400000000005820ETX_ACK_ufis................2014120909084400000000005821ETX_ACK_ufis................2014120909084400000000005822ETX_ACK_ufis................2014120909084400000000005823ETX_");
                    }
...
static int SendBatchFlights(void)
{
	...
	char pclAdidValue[16] = "\0";
	char pclFltiValue[16] = "\0";
    char pclGta1Value[16] = "\0";
	char pclGtd1Value[16] = "\0";
    char pclTifaValue[16] = "\0";
	char pclTifdValue[16] = "\0";
	char pclFlnoValue[16] = "\0";
    char pclOnblValue[16] = "\0";
	char pclOfblValue[16] = "\0";

	char pclArrRefValue[16] = "\0";
	char pclDepRefValue[16] = "\0";

	char pclFieldList[1024] = "URNO,ADID,FLNO,GTA1,GTD1,FLTI,TIFA,TIFD,ONBL,OFBL";
    char pclDataSent[1024] = "\0";
	char pclAckSent[1024] = "\0";
	char pclSqlBuf[2048] = "\0",pclSqlData[2048] = "\0",pclWhere[2048] = "\0";
	...
	strcat(pclFieldList,",");
	strcat(pclFieldList,pcgTimeWindowRefField_AFTTAB_Arr);
	strcat(pclFieldList,",");
	strcat(pclFieldList,pcgTimeWindowRefField_AFTTAB_Dep);

    ilNoEle = GetNoOfElements(pclFieldList, ',');
	GetFlightWhereClause(pclWhere);
	GetFlightFullQuery(pclSqlBuf,pclFieldList,pclWhere);
	...
	while(sql_if(slFuncCode, &slLocalCursor, pclSqlBuf, pclSqlData) == DB_SUCCESS)
    {
        ...
        dbg(TRACE,"%s <%d> pclSqlData<%s>", pclFunc, ilCount, pclSqlData);

        if( getFieldValue(pclFieldList, pclSqlData, "URNO", pclUrnoValue) == RC_FAIL )
            continue;

        if( getFieldValue(pclFieldList, pclSqlData, "ADID", pclAdidValue) == RC_FAIL )
            continue;
        if( getFieldValue(pclFieldList, pclSqlData, "FLNO", pclFlnoValue) == RC_FAIL )
            continue;

        if( getFieldValue(pclFieldList, pclSqlData, pcgTimeWindowRefField_AFTTAB_Arr, pclArrRefValue) == RC_FAIL )
            return RC_FAIL;

        if( getFieldValue(pclFieldList, pclSqlData, pcgTimeWindowRefField_AFTTAB_Dep, pclDepRefValue) == RC_FAIL )
            return RC_FAIL;

        strcpy(rlSentMsg.pclAdid,pclAdidValue);
        strcpy(rlSentMsg.pclFlno,pclFlnoValue);

        if(adidFilter(pclAdidValue) == RC_FAIL)
        {
            continue;
        }
        switch(pclAdidValue[0])
        {
            case 'A':
                if( isInTimeWindow(pclArrRefValue, pcgTimeWindowLowerLimit, pcgTimeWindowUpperLimit) == RC_FAIL )
                {
                ...
                if( getFieldValue(pclFieldList, pclSqlData, "GTA1", pclGta1Value) == RC_FAIL )
                    continue;
                if( getFieldValue(pclFieldList, pclSqlData, "TIFA", pclTifaValue) == RC_FAIL )
                    continue;

                strcpy(rlSentMsg.pclGate,pclGta1Value);
                strcpy(rlSentMsg.pclBestTime,pclTifaValue);
                break;
            case 'D':
                if( isInTimeWindow(pclDepRefValue, pcgTimeWindowLowerLimit, pcgTimeWindowUpperLimit) == RC_FAIL )
                {
                ...
                if( getFieldValue(pclFieldList, pclSqlData, "GTD1", pclGtd1Value) == RC_FAIL )
                    continue;
                if( getFieldValue(pclFieldList, pclSqlData, "TIFD", pclTifdValue) == RC_FAIL )
                    continue;

                strcpy(rlSentMsg.pclGate,pclGtd1Value);
                strcpy(rlSentMsg.pclBestTime,pclTifdValue);
                break;
            default:
                dbg(TRACE,"<%s>: Invalid ADID <%s>",pclFunc,pclAdidValue);
                return RC_FAIL;
                break;
        }
        if( strstr(pclAllValidGates,rlSentMsg.pclGate) == 0 )
        {
            dbg(TRACE,"The retrieved gate<%s> is invalid which is not in the basic data in valid duration",pclFunc,rlSentMsg.pclGate);
            continue;
        }

        if( getFieldValue(pclFieldList, pclSqlData, "FLTI", pclFltiValue) == RC_FAIL )
            continue;
        if( getFieldValue(pclFieldList, pclSqlData, "ONBL", pclOnblValue) == RC_FAIL )
            continue;
        if( getFieldValue(pclFieldList, pclSqlData, "OFBL", pclOfblValue) == RC_FAIL )
            continue;

        strcpy(rlSentMsg.pclFlightID,pclAdidValue);
        strcpy(rlSentMsg.pclOnbl,pclOnblValue);
        strcpy(rlSentMsg.pclOfbl,pclOfblValue);

        memset(pclDataSent,0,sizeof(pclDataSent));
		memset(pclAckSent,0,sizeof(pclAckSent));

        ShowMsgStruct(rlSentMsg);
        BuildSentData(pclDataSent,rlSentMsg);
        dbg(TRACE,"\n\n<%s> pclDataSent<%s>",pclFunc,pclDataSent);
        StoreSentData(pclDataSent,pclUrnoValue,rlSentMsg,pclRecordURNO,"BATCH","INI");
    ...
    dbg(TRACE,"%s Total Number of Flights <%d>",pclFunc, ilCount);
...
static int StoreSentData(char *pcpDataSent, char *pcpUaft, SENT_MSG rpMsg, char *pcpRecordURNO, char *pcpType, char *pcpStat)
{
    ...
    sprintf(pclDataList,"%s,'%s','%s','%s','%s','%s','%s','%s','%s','%s'",pclUrno,pcpUaft,rpMsg.pclFlno,rpMsg.pclAdid,rpMsg.pclGate,rpMsg.pclBestTime,pcpType,pcpDataSent,pcpStat,pclCurrentTime);
