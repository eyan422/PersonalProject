  ...
  static char mks_version[] = "@(#) "UFIS_VERSION" $Id: Ufis/_Standard/_Standard_Server/Base/Server/Kernel/hwecon.c 1.3 12/20/2012 10:29:38 AM fya Exp  $";
...
static int	igReconMax = 3;             /* global socket reconnect Max Times */
static int	igReSendMax = 10;           /* global socket resend Max Times */
...
static int HandleInternalData()
{
	...
	else if (strcmp(cmdblk->command,"ACK") == 0)
    {
        igSckWaitACK = FALSE;
        igSckTryACKCnt = 0;
        ilRC = RC_SUCCESS;
...
static int GetConfig()
{
  ...
  */
  ...
  ilRC = iGetConfigEntry(pcgConfigFile,"MAIN","PORT",CFG_STRING,pclSelection);
  if(strlen(pclSelection)==0)
  {
  	dbg(TRACE,"PORT is null<%s>",pclSelection);
  }
  else
  {
  	igPort = atoi(pclSelection);
  	if(igPort <= 0 )
  	{
  		dbg(TRACE,"PORT is invalid<%d>",igPort);
  	}
  	else
  	{
  		dbg(TRACE,"PORT is valid<%d>",igPort);
  	}
  }
...
static int poll_q_and_sock() 
{
	...
	char pclLastTimeSendingHeartbeat[128] = "\0";
	char pclCurrentTime[128] = "\0";
	char pclTmpCurrent[128] = "\0";
	char pclTmpLast[128] = "\0";
	int ilTmpCurrent = 0;
	int ilTmpLast = 0;
...
//		dbg(TRACE,"+++++++++++++++++++++++++++++");
  	...
  	//	dbg(TRACE,"pclCurrentTime<%s>,pclLastTimeSendingHeartbeat<%s>",pclCurrentTime,pclLastTimeSendingHeartbeat);
  		...
  		strcpy(pclTmpCurrent,pclCurrentTime+8);
  		strcpy(pclTmpLast,pclLastTimeSendingHeartbeat+8);
  		
  		ilTmpCurrent = atoi(pclTmpCurrent);
  		ilTmpLast = atoi(pclTmpLast);
  		...
  		if((ilTmpCurrent - ilTmpLast) > 5)
  		{
  			dbg(TRACE,"<%s>(ilTmpCurrent<%d> - ilTmpLast<%d>) > 5",pclFunc,ilTmpCurrent,ilTmpLast);
				...
				else if (ilRc == RC_SUCCESS)
				{
					...
					dbg(TRACE,"<%s>SendHeratbeatMsg succed",pclFunc);
				}
				else if (ilRc == RC_SENDTIMEOUT)
				{
...
static int SendHeartbeatMsg(char *pcpLastTimeSendingTime)
{
	...
	dbg(DEBUG,"<%s> Send Heartbeat Msg igMsgNoForHeartbeat<%d>pclFormatTime<%s>",pclFunc,igMsgNoForHeartbeat,pclFormatTime);
	...
	if(ilRC == RC_SUCCESS)
	{
		dbg(TRACE,"<%s> HeartBeat sending successfully",pclFunc);
	}
	else if(ilRC == RC_SENDTIMEOUT)
	{
		dbg(TRACE,"<%s> HeartBeat sending timeout",pclFunc);
...
static int ReceiveACK(int ipSock,int ipTimeOut)
{
  ...
  rlTimeout.tv_usec = 0;
	...
	ilFound = select( ipSock+1, (fd_set *)&readfds, NULL, NULL, &rlTimeout );
...
static int Receive_data(int ipSock,int ipTimeOut)
{
    ...
    if (igSckWaitACK == TRUE)
	    rlTimeout.tv_sec = 2;  /* blocked for 2 seconds for ACK*/
    else
	    rlTimeout.tv_sec = ipTimeOut;  /* blocked for r seconds */
				...
				if(FD_ISSET( ipSock, &readfds))
				{
		    	    ...
		    	    if( ilNo > 0 )
		            {
		      	        if(strstr(pclRecvBuffer,"heartbeat") == 0)
		      	            dbg(DEBUG,"<%s> Len<%d>Complete message<%s>",pclFunc,ilNo,pclRecvBuffer);
	      	            if (strstr(pclRecvBuffer,"ack")!=0)
				        {
					        dbg(DEBUG,"<%s> Received the ack message",pclFunc);
                            igSckWaitACK = FALSE;
                            igSckTryACKCnt = 0;
			      	        ilRC = SendCedaEvent(igModID_ConSTB,0,mod_name,"CEDA",pcgTwStart,pcgTwEnd,"ACK","","","","","","",NETOUT_NO_ACK);
			      	        //ilRC = SendCedaEvent(igModID_ConSTB,0,mod_name,"CEDA",pcgTwStart,pcgTwEnd,"RACK","","","","","","",NETOUT_NO_ACK);
			      	        //ilRC = SendCedaEvent(igModID_ConMgr,0,mod_name,"CEDA",pcgTwStart,pcgTwEnd,"ACK","","","","","","",NETOUT_NO_ACK);
			      	        ilRC = SendCedaEvent(igModID_ConMgr,0,mod_name,"CEDA",pcgTwStart,pcgTwEnd,"RACK","","","","","","",NETOUT_NO_ACK);
					        // return RC_SUCCESS;
		      	        }
		      	        else if(strstr(pclRecvBuffer,"heartbeat") == 0)
		      	        {
						    ...
						    SendAckMsg();
			      	        ilRC = SendCedaEvent(igModID_Rcvr,0,mod_name,"CEDA",pcgTwStart,pcgTwEnd,"XMLI","","","",pclRecvBuffer,"","",NETOUT_NO_ACK);
	                        if (ilRC == RC_SUCCESS)
	                        {
...
static int tcp_socket(int	*ilAcceptSocket)
{
  ...
  dbg( TRACE,"<%s>Going to run TCP(Client) connecting to IP<%s> at PORT<%d>",pclFunc,pcgIP,igPort);
  ...
  memset( &rlSin, 0, sizeof(rlSin) );
	rlSin.sin_family = AF_INET;
  rlSin.sin_port = htons(igPort);
  
  dbg(TRACE,"<%s>pcgIP<%s>",pclFunc,pcgIP);
	inet_pton( AF_INET, pcgIP, &rlSin.sin_addr );
	dbg(TRACE,"<%s>+++",pclFunc);
...
static int SendAckMsg()
{
	...
	char pclDataBuf[M_BUFF];
	char pclFormatTime[128]="\0";
	char pclLastTimeSendingTime[128] = "\0";
	...
	dbg(DEBUG,"<%s> Send Ack Msg igMsgNoForACK<%d>pclFormatTime<%s>",pclFunc,igMsgNoForACK,pclFormatTime);
	
	sprintf(pclDataBuf,pcgSendAckFormat,igMsgNoForACK++,pclFormatTime,pclFormatTime);
	ilRC = Send_data(igSock,pclDataBuf);
...
static int Sockt_Reconnect(void)
{
    ...
    CloseTCP();
	dbg(TRACE,"Keep trying to connect <%s><%d>",pcgIP,igPort);
