...
# 20140730 - special version for SATS POC 5.1, to force FLIGHT to auto fill
#		the A-CDM fields.
#		To force all the records to be updated, select based on FLDA
#		only, then perform two updates on FTYP, one to 'S' then next
#		one to 'O'. One reason is because most of the records are already
#		'O'.
...
touch /ceda/tmp/CountStar.txt
rm /ceda/tmp/CountStar.txt
touch /ceda/tmp/LogAndURNO_List.txt
...
do
	...
	set echo off
	spool spool_file
	SELECT URNO FROM AFTTAB WHERE (FLDA LIKE '$Date%') AND FTYP='O';
	spool off
	exit;
EOF
`
	...
	do
		...
		if [ -n "$URNO" ];then
			cput 0 flight UFR "URNO,FTYP" "$URNO,S" "WHERE URNO=$URNO" 7
                        sleep 1
		else
			echo "URNO is null"
			break
...
!
`
     echo "SELECT COUNT(*) FROM AFTTAB WHERE (FLDA LIKE '$Date%') AND FTYP='O' > Number=$CountStarAfterS" >> /ceda/tmp/CountStar.txt
...
!
`
     echo "SELECT COUNT(*) FROM AFTTAB WHERE (FLDA LIKE '$Date%') AND FTYP='S' > Number=$CountStarAfterO" >> /ceda/tmp/CountStar.txt
